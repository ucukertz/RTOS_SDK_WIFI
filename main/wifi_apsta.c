...
static const u8_t WIFI_AP_MAX_CONN = 2;
...
// Switch Wi-Fi mode to AP+STA - Newest STA credential configuration will be applied
void wifi_to_apsta()
{
    wifi_config_t ap_config = {0};
    wifi_config_set_ap(&ap_config);
...
void wifi_init()
{
    ...
    tcpip_adapter_ip_info_t ap_static_ip;
    IP4_ADDR(&ap_static_ip.ip, 192,168,4,1);
	IP4_ADDR(&ap_static_ip.gw, 192,168,4,1);
	IP4_ADDR(&ap_static_ip.netmask, 255,255,255,0);

    tcpip_adapter_dhcps_stop(TCPIP_ADAPTER_IF_AP);
    tcpip_adapter_set_ip_info(TCPIP_ADAPTER_IF_AP, &ap_static_ip);
